    ...
    <div className="bg-card border border-border rounded-xl p-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-4">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-4 text-xs text-muted-foreground">
          ...
          <span>⭐ Best: {bestDay?.count ?? 0}</span>
          <span>❌ {missedDays} missed</span>
        </div>
      </div>
      <div className="flex gap-[2px] sm:gap-[3px] overflow-x-auto pb-2 -mx-2 px-2">
        {weeks.map((week, wi) => (
          <div key={wi} className="flex flex-col gap-[2px] sm:gap-[3px]">
            {week.map((day) => (
              <div
                key={day.date}
                className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-[2px] ${getIntensityClass(day.count)}`}
